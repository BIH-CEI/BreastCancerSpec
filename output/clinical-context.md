# Clinical Context - Breast Cancer Specification v0.1.0
...
The ICCR datasets model the lymph node specimen report as a **standalone report** with its own structured dataset, separate from the breast specimen report. This IG provides an example following this approach (see [SLN-Biopsie Axilla](lymph-node-specimens.md)).
...
1. The pathologist issues a**preliminary report**(`DiagnosticReport.status = preliminary`) with the frozen section diagnosis (e.g. "SLN tumour-free").
1. Based on the negative result, no axillary dissection is performed.
1. After paraffin embedding and staining, the**final report**integrates the breast specimen findings, the definitive SLN assessment, biomarkers and TNM staging into a single document.
...
1. Both the breast specimen and all lymph node specimens (SLN + dissection) are reported together in a**single final report**.
...
1. Paraffin sections later reveal metastatic involvement in the SLN.
1. A**re-operation**(axillary dissection) is performed, and the additional specimens are reported as an**addendum or follow-up report**assigned to the same case.

#### Representation in this IG

This IG provides both models: the **ICCR-oriented model** (standalone SLN report, see [SLN-Biopsie Axilla](lymph-node-specimens.md)) and an **[integrated report](integrated-report.md)** combining the breast specimen and lymph node findings into a single Bundle under one accession number. The ICCR model serves international interoperability and structured data exchange, while the integrated model aligns with routine German pathology practice.
